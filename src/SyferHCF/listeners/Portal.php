...
class Portal implements Listener{
    ...
    public function Portal__construct(Loader $plugin){
        $this->plugin = $plugin;
    }
    ...
    public function onInteract(PlayerInteractEvent $event)
    {
        ...
        if($item->getId() == ItemIds::DIAMOND_HOE and $item->getName() == TextFormat::RESET . TextFormat::DARK_RED . "Crowbar"){
            if($spawn->distance($block) < 100 and !$player->isOp()){
    ...
    public function onPlace(BlockPlaceEvent $event)
    {
        ...
        if($block->getId() == 120){
            if($spawn->distance($block) < 400 and !$player->isOp()){
                $player->sendMessage(TextFormat::RED . "You can't build near the spawn!");
                $event->setCancelled(true);
                return;
            }

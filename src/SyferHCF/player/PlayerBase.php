...
class PlayerBase {
	...
	public static function create(String $playerName) : void {
		if(!file_exists(Loader::getInstance()->getDataFolder()."players".DIRECTORY_SEPARATOR."{$playerName}.yml")){
            ...
            $config->setAll(["kills" => null, "deaths" => null, "lives" => null, "lives_claimed" => false, "reclaim" => null, "koth_host" => null, "citadel_host" => null, "brewer" => null,"balance" => null]);
            $config->save();
		}
	}
   ...
   /**
	 ...
	 * @return Int
	 */
	public static function getDeaths(String $playerName) : Int {
		return self::getData($playerName)->get("deaths") === null ? 0 : self::getData($playerName)->get("deaths");
	}

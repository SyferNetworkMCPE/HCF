...
use BullHCF\{listeners\Faction, Loader, Factions};
use BullHCF\player\{Player, PlayerBase};

use BullHCF\utils\Time;

use BullHCF\Task\{InvitationTask, TeleportHomeTask, TeleportStuckTask};
use BullHCF\Task\updater\UpdaterTask;

use BullHCF\API\System;
use BullHCF\utils\Tower;
        ...
        switch($args[0]){
            ...
            case "info":
                if(empty($args[1])){
                    ...
                    $players = Factions::getMaxPlayers(Factions::getFaction($senderName)) === null ? "Error 404 report in discord" : Factions::getMaxPlayers(Factions::getFaction($senderName));$onlinePlayers = Factions::getListPlayers(Factions::getFaction($senderName)) === null ? "No members connected" : Factions::getListPlayers(Factions::getFaction($senderName));
                    ...
                    $sender->sendMessage(str_replace(["&", "{home}", "{factionName}", "{onlinePlayers}", "{players}", "{maxPlayers}", "{currentDtr}", "{leader}", "{co_leader}", "{balance}", "{timeRegen}", "%n%"], ["§", $homeLocation, Factions::getFaction($senderName), $onlinePlayers, $players, $maxPlayers, $dtr, $leader, $co_leader, $balance, $timeRegen, "\n"], Loader::getConfiguration("messages")->get("faction_information")));
                }else{
                	$player = Loader::getInstance()->getServer()->getPlayer($args[1]);
                	if($player instanceof Player){
	                    ...
	                    $sender->sendMessage(str_replace(["&", "{home}", "{factionName}", "{onlinePlayers}", "{players}", "{maxPlayers}", "{currentDtr}", "{leader}", "{co_leader}", "{balance}", "{timeRegen}", "%n%"], ["§", $homeLocation, Factions::getFaction($player->getName()), $onlinePlayers, $players, $maxPlayers, $dtr, $leader, $co_leader, $balance, $timeRegen, "\n"], Loader::getConfiguration("messages")->get("faction_information")));
                	}else{
                  	    if(!Factions::isFactionExists($args[1])){
	                        $sender->sendMessage(str_replace(["&", "{factionName}"], ["§", $args[1]], Loader::getConfiguration("messages")->get("faction_not_exists")));
	                        return;
	                    ...
	                    $players = Factions::getMaxPlayers($args[1]) === null ? "Error 404 report in discord" : Factions::getMaxPlayers($args[1]);$onlinePlayers = Factions::getListPlayers($args[1]) === null ? "No members connected" : Factions::getListPlayers($args[1]);
	                    ...
	                    $sender->sendMessage(str_replace(["&", "{home}", "{factionName}", "{onlinePlayers}", "{players}", "{maxPlayers}", "{currentDtr}", "{leader}", "{co_leader}", "{balance}", "{timeRegen}", "%n%"], ["§", $homeLocation, $args[1], $onlinePlayers, $players, $maxPlayers, $dtr, $leader, $co_leader, $balance, $timeRegen, "\n"], Loader::getConfiguration("messages")->get("faction_information")));
	                }
				}
            break;
            default:
